...
export async function getFeaturedProjects(limit = 3) {
  const { rows } = await pool.query(
    ...
    FROM projects
    WHERE is_featured IS TRUE               -- safer than = true
...
export async function createProject(data) {
  const { title, slug, summary = '', body = '', thumbnail_url = null, tags = [] } = data;
  const { rows } = await pool.query(`
    INSERT INTO projects (title, slug, summary, body, thumbnail_url, tags, published_at)
